...
class Config:
    def __init__(self):
        ...
        self.epochs = 15  # 增加最大epoch数，让早停机制决定何时停止
...
def train_model(model, train_loader, val_loader, device, tokenizer):
    ...
    for epoch in range(config.epochs):
        if early_stop:
            print(f"早停触发，停止训练。最佳epoch: {epoch - epochs_no_improve}")
            break
        ...
        print(f"Epoch {epoch + 1} - 时间: {epoch_time:.2f}秒")
        print(
            f"  Losses: Total: {avg_loss:.4f}, NER: {avg_ner_loss:.4f}, Group: {avg_group_loss:.4f}, Hate: {avg_hate_loss:.4f}")
        ...
        # 早停机制 - 监控Quad Soft Acc
        current_quad_soft_acc = quad_metrics['quad_soft_acc']
        if current_quad_soft_acc > best_quad_soft_acc + config.min_delta:
            print(f"  四元组软匹配准确率提升 ({best_quad_soft_acc:.4f} → {current_quad_soft_acc:.4f})")
            best_quad_soft_acc = current_quad_soft_acc
            ...
            epochs_no_improve += 1
            print(f"  四元组软匹配准确率未提升，已连续 {epochs_no_improve}/{config.patience} 个epoch")
...
def main():
    ...
    # 保存模型
    torch.save(model.state_dict(), 'hate_model5.pt')
    print("模型已保存到 hate_model5.pt")
    ...
    # 保存预测结果
    with open('demo5.txt', 'w', encoding='utf-8') as f:
        for pred in predictions:
            f.write(pred + '\n')

    print("预测结果已保存到 demo5.txt")
